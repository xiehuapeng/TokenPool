...
def _upstream_error(response: httpx.Response, body: bytes) -> GatewayError:
    message = "Zhipu GLM upstream request failed"
    try:
...
class GLMProvider(BaseProvider):
    ...
    def _headers(self) -> dict[str, str]:
        api_key = get_settings().glm_api_key.get_secret_value()
        if not api_key:
            raise GatewayError(
                "Zhipu GLM Provider is not configured",
                status_code=503,
    ...
    @staticmethod
    def _url() -> str:
        return f"{get_settings().glm_base_url.rstrip('/')}/chat/completions"

    @staticmethod
    def _models_url() -> str:
        return f"{get_settings().glm_base_url.rstrip('/')}/models"
    ...
    async def list_models(self, *, timeout_seconds: int) -> list[ProviderModel]:
        async with self._client(timeout_seconds) as client:
            try:
                ...
                raise GatewayError(
                    "Zhipu GLM model listing timed out",
                    status_code=504,
                    error_type="upstream_error",
                ...
                raise GatewayError(
                    "Unable to connect to Zhipu GLM model listing",
                    status_code=502,
                    error_type="upstream_error",
        ...
        try:
            ...
            raise GatewayError(
                "Zhipu GLM returned an invalid model list",
                status_code=502,
                error_type="upstream_error",
    ...
    async def chat_completion(
        ...
        try:
            ...
            raise GatewayError(
                "Zhipu GLM returned invalid JSON",
                status_code=502,
                error_type="upstream_error",
                code="invalid_upstream_response",
            ) from exc

...
如果有多个 Key（如不同工具各用一把），可在工作台给单个 Key 单独绑定首选模型，
该 Key 的调用会优先使用 Key 级偏好，其次才是用户级偏好。

Model 也可以直接填写你有权限的真实模型 ID（如 `glm-5.3-flash`），此时不经过
工作台偏好路由，直接调用对应模型。日常问答、资料整理等轻量任务建议优先使用
低成本模型（如 `glm-5.3-flash`、`deepseek-v4-flash`），复杂任务再换高规格模型。
...
## 最近一次上线变更（2026-08-27 / 2026-08-31）

08-25 之后又上线两批变更，均已部署生产并验证：

### 2026-08-27（用户侧体验）

- 用户「用量」页新增每个模型的 Token 与实时费用，支持按时间和模型筛选。
- 用户「模型」页按 Provider 分组，并展示模型描述（含 GLM 系列说明）。
- 修正 GLM 模型能力标记，新增 `glm-5.3-flash`。

### 2026-08-31（视觉模型扩容）

- 视觉能力标记从 2 个模型扩至 6 个（新增 `glm-5.3-flash`、`qwen3.7-plus`、
  `kimi-k2.7-code`、`kimi-k2.7-code-highspeed`）。
- 含图片请求命中非视觉模型时的自动切换（vision fallback）覆盖面同步扩大。
- 该改动经文件 md5、systemd 启动时间、数据库三方对照确认已在生产生效，
  代码提交 `dbcaf4b` 并推送 GitHub `main`。

### 2026-08-31（计价核验与限时折扣展示）

- 全部 18 条模型计价与 DeepSeek、智谱、Moonshot、阿里云百炼官网逐项核对，
  均一致；`glm-5.3-flash` 当前为限时半价（0.4/1.4 元），折扣至 2026-09-09，
  计价备注已标注，9 月 10 日起需恢复原价输入 0.8/输出 2.8。
- 用户模型列表价格区新增「限时半价」标签，悬停展示折扣截止等完整备注；
  标签由计价备注数据驱动，后续新折扣无需改代码。提交 `fd88250`（计价种子）、
  `2ced6cc`（前台标签）已推送。

## 最近一次上线变更（2026-09-01 至 2026-09-09）

08-31 之后又上线四批变更（均已部署生产并验证），另有一批待部署：

### 2026-09-01（模型列表重构与退役加固）

- 模型页重构，支持长上下程分档计价展示与模型退役流程（提交 `5ed1924`）。
- 新增 `qwen3.8-flash`（含视觉标记），退役 `qwen3.7-max` 并自动迁移用户
  模型偏好（提交 `5208a94`）。
- 修复退役模型时未先删除关联 `ModelPricing` 导致 PostgreSQL 约束冲突、
  后端连续重启的问题（提交 `bb95f47`）。

### 2026-09-02（模型页视觉调整）

- 供应商分组标题强化，`glm-5.3-flash` 显示绿色「推荐」标记，仪表盘模型
  描述对齐（提交 `6a41e7d`、`5afb96e`、`4af88d6`）。

### 2026-09-03（GLM 流兼容与管理后台增强）

- 规范化 GLM SSE 兼容处理，新增流式观测指标（提交 `7a7b03f`）。
- 管理后台新增邀请码绑定展示、按用户 Key 汇总和统计自动刷新（提交
  `5744420`，迁移 `20260903_0007_user_invite_binding`）。

### 2026-09-08（审查修复与滚动升级加固）

本轮修复代码审查发现的 F1-F12 共 12 项问题，并完成生产无感双实例滚动升级
（提交 `24c481b`）：

- 修复流式计费：流内 error 终态、无 DONE 的 EOF 分类、首非空正文指标、
  Provider 错误读取清理。
- 账单补算对混合来源批次整批拒绝且不修改旧账；API Key 创建按用户原子锁
  并排除过期 Key；服务重启后中断请求按进程锁归属恢复。
- 登录/注册校验分离；TRAE 配置说明补充完整 URL；管理端最新请求保护。
- 前端原子部署脚本增加 tar/依赖校验、旧哈希资源保留与失败回滚；滚动
  升级流程固化到 `deploy/ROLLING_UPGRADE.md`。
- 新增 `usage_runtime` 运行时记录（迁移 `20260908_0008_usage_runtime`，
  仅新增 nullable 字段，不重算历史账目）。
- 生产采用候选实例承接新流量、旧实例自然排空的方式升级，未中断活动流。

### 2026-09-09（上架 DeepSeek 视觉实验模型，待部署）

- 新增 `deepseek-v4-flash-vision-exp`（视觉标记、峰谷计价与 flash 相同，
  图片按尺寸折算为输入 Token 计费），DeepSeek 用户可直选 1M 上下文视觉模型。
- bootstrap 种子新增「同步采纳」逻辑：官方同步任务提前发现的新模型在下次
  启动时自动补齐视觉标记与计价并随 Provider 开启（一次性，不覆盖管理员
  后续启停选择）。
- 视觉自动切换候选新增该模型；测试基线 121 passed / 1 skipped。

### 2026-09-10（glm-5.3-flash 恢复原价）

- 限时半价（0.4/0.115/1.4）按期结束，恢复智谱官网原价：输入 0.8、缓存输入
  0.23、输出 2.8；计价备注改为「智谱官网价，不分档」，用户模型列表的
  「限时半价」标签随备注更新自动消失。
- 种子数据（bootstrap）与生产数据库同步更新；计价为实时查库，改价即时
  生效，历史账目与原始 Token 数据不受影响。
- 修复生产 `/opt/tokenpool/.env` 首行 `APP_NAME=Team AI Gateway` 缺少引号
...
4. PostgreSQL 定时备份与异机恢复演练。
5. 多 API Key 轮询和 Provider 自动故障切换。
